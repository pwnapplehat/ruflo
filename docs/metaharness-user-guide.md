...
**MetaHarness integration in ruflo `3.12.1+`.** Ten CLI subcommands, nine MCP tools, three CI workflows, and a dedicated `ruflo eject` command â€” all wired to the upstream `metaharness` / `@metaharness/*` ecosystem with **graceful degradation** when those optional packages aren't installed.

Quick links: [Quick start](#quick-start) Â· [10 CLI subcommands](#cli-subcommands) Â· [9 MCP tools](#mcp-tools) Â· [Architectural constraints](#architectural-constraints-adr-150) Â· [Workflows](#common-workflows) Â· [Troubleshooting](#troubleshooting) Â· [ADR-152 similarity search](#adr-152-Â§31-genome-similarity-search) Â· [Eject](#ruflo-eject)

---

## What is MetaHarness?

`metaharness` is a sibling agent-harness scaffolding system designed by the same author as ruflo. Where ruflo *is* a harness, metaharness analyzes harnesses â€” scoring readiness, mapping MCP surfaces, threat-modeling, fingerprinting genome characteristics, and detecting drift over time. ADR-150 integrates it as a first-class subsystem so you can audit and characterize ruflo (or any harness) from the same CLI.

The integration is **strictly optional**. Per [ADR-150](../v3/docs/adr/ADR-150-metaharness-integration-surfaces.md) constraint #4, ruflo remains fully operational even when every `@metaharness/*` package is uninstalled â€” every command degrades gracefully with a clear `degraded: true` payload instead of crashing.
...
# Score two harnesses' similarity (ADR-152 Â§3.1)
...
| 5 | `oia-audit` | Composite audit â†’ memory | `{timing, composite: {worst}, components, fingerprint, alert, persisted}` |
...
| 8 | `similarity` | ADR-152 Â§3.1 weighted similarity | `{overall, components: {cosine, categorical, jaccard}, perDimension?}` |
...
Returns five numeric dimensions (0â€“100):

- **harnessFit** â€” overall readiness composite
- **compileConfidence** â€” build/test signal strength
- **taskCoverage** â€” breadth of declared agent roles
- **toolSafety** â€” MCP policy posture
- **memoryUsefulness** â€” persistence + retrieval characteristics

Plus `estCostPerRunUsd`, `recommendedMode` (`CLI` / `CLI + MCP`), `archetype`, `template`.

### `genome` â€” 7-section categorical
...
### `mcp-scan` â€” MCP security

```bash
npx ruflo metaharness mcp-scan --path . --fail-on high
```

Reads `.mcp/servers.json` + `.harness/claims.json` and runs static analysis. Finding shape is normalized to `{severity, message, title?, detail?, id?}` â€” same fields whether upstream emitted JSON or our text-parser fell back.

`--fail-on {low|medium|high}` sets the `alert.triggered` floor.

### `threat-model` â€” Enterprise threat report
...
### `oia-audit` â€” Composite audit â†’ memory
...
### `audit-list` â€” Enumerate records
...
Returns composite worst-severity delta + per-component status change + introduced/cleared findings + (ADR-152 Â§3.1) **structural distance** when both records carry a `fingerprint`.

Accepts memory keys OR direct file paths (`--baseline /path/to/json.json`) â€” useful for diffing CI artifacts.

### `similarity` â€” ADR-152 Â§3.1 weighted similarity
...
Returns `overall âˆˆ [0,1]` plus per-component breakdown:
...
See [ADR-152 Â§3.1 below](#adr-152-Â§31-genome-similarity-search) for math + use cases.

### `drift-from-history` â€” One-command drift

```bash
# Slowest path â€” discovers the most recent audit in memory
npx ruflo metaharness drift-from-history --threshold 0.95

# Fast path â€” skip audit-list (~14Ã— faster)
npx ruflo metaharness drift-from-history \
  --baseline-key audit-2026-06-15T... \
  --threshold 0.95

# Fastest path â€” skip memory entirely (~19Ã— faster)
npx ruflo metaharness drift-from-history \
...
|---|---|---|---|
| Slow | (none) | ~26 s | Interactive â€” let it discover the baseline |
...
`success === false` is the source of truth for "this should block downstream action" â€” `exitCode` is also surfaced for shell-script consumers but the MCP layer uses `success`.
...
| 4 | **CI gate** | `.github/workflows/no-metaharness-smoke.yml` enforces 1â€“3 by static grep + runtime drill on every PR |
...
â€¦and exits 0. Downstream tooling can branch on `degraded` to fall back or skip.
...
The composite audit writes a record keyed by ISO timestamp. `drift-from-history` discovers it via `audit-list`, runs a fresh audit, diffs the fingerprints via ADR-152 Â§3.1 similarity, and alerts when:
...
### Template ranking (ADR-151 Â§3.2)
...
## ADR-152 Â§3.1 Genome Similarity Search
...
`overall = w_c Â· cosine + w_k Â· categorical + w_j Â· jaccard`, all in `[0, 1]`.

**Verdict thresholds:**

| overall | verdict |
|---|---|
| â‰¥ 0.95 | `near-identical` |
| â‰¥ 0.85 | `minor-drift` |
| â‰¥ 0.5 | `moderate-drift` |
| < 0.5 | `major-drift` |
...
`--strict` exit 1 if any criterion fails â€” the **promotion gate** before swapping the bandit out for the neural router in production.
...
```bash
# Dry-run (default) â€” prints the plan and exits without writing
...
**Safety gate:** refuses any `--target` inside the calling repo. The default target is `/tmp/ruflo-eject-<ts>-<name>/` â€” a fresh location to prevent eject-on-top-of-source accidents.
...
Reports installed/missing status for `@metaharness/router`, `metaharness`, `@metaharness/kernel`, plus the plugin script directory location. Always exits 0 â€” doctor reports state, never blocks.
...
The optional `metaharness` / `@metaharness/*` packages aren't in `node_modules`. Per ADR-150 constraint #3 this is a **valid degraded mode** â€” ruflo still works, you just won't get score/genome/etc. results. To enable them:

```bash
npm install -D metaharness@latest @metaharness/router@latest
```

(Or accept the degraded mode â€” ruflo doesn't *require* metaharness for any non-metaharness command.)
...
Check the namespace â€” `oia-audit` persists to `metaharness-audit` by default. If you've overridden `AUDIT_LIST_NAMESPACE`, set it for `audit-list` too:
...
Expected â€” `oia-audit` spawns 5 sub-audits in parallel and each shells out to `npx metaharness <cmd>`. Cold-cache npx warmup is ~25 s per process. Mitigations:
...
Usually transient network ECONNRESET on sharp / onnxruntime-node postinstall. Retry the install â€” the cron-fire workflows ship with `npm_config_fetch_retries=5` so most flakes auto-recover.
...
- **ADR-152 Â§3.1 similarity**: [`v3/docs/adr/ADR-152-genome-similarity-search.md`](../v3/docs/adr/ADR-152-genome-similarity-search.md)
- **Tracking issue**: [#2399](https://github.com/pwnapplehat/ruflo/issues/2399)
...
- `ruvnet/agent-harness-generator#15` â€” CLI schema mismatch (downstream workaround via `runMetaharness` routing in place)
- `ruvnet/agent-harness-generator#16` â€” `mcp-scan` text-only output (downstream `parseMcpScanText` parser donated as MIT contribution)

Both are tracked in [ADR-150 Â§"Cross-references"](../v3/docs/adr/ADR-150-metaharness-integration-surfaces.md).
