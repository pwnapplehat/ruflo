# BEIR Public-Benchmark Results â€” ruflo
...
|---|---:|---:|---|---|---:|---|---|---:|---:|---:|---|
| **NFCorpus** | 3,633 | 323 | direct dense (no rerank) | BGE-base-en-v1.5 (110M) | **0.352** | [0.317, 0.387] | +0.027 â†‘ (n.s.) | -0.028 â†“ BGE-large 0.380 (n.s.) | **2/11** | 388 ms | `beir-nfcorpus-bge-latest.json` |
| **NFCorpus** | 3,633 | 323 | pure BM25 (silent hash-fallback path) | _no real dense_ | 0.289 | _n/a_ | -0.036 â†“ | -0.091 â†“ | 11/11 | 950 ms | `beir-nfcorpus-2026-05-30T19-16-23-024Z.json` |
| **SciFact** | 5,183 | 300 | direct dense (no rerank) | BGE-base-en-v1.5 (110M) | 0.626 | [0.577, 0.672] | **-0.053 â†“ (p<0.05)** | -0.096 â†“ BGE-large 0.722 (p<0.05) | 10/11 | 410 ms | `beir-scifact-bge-latest.json` |

### Bootstrap CI summary (per ADR-086, 10k resamples, seed=42)

The 95% confidence intervals tell the rigorous story. On NFCorpus, we beat BM25 by 0.027 *point estimate* but the CI overlaps the baseline (n.s. at p<0.05) â€” the "rank-2" headline is a single-realisation outcome, not a statistically distinguishable win. On SciFact, we lose to BM25 by 0.053 and the CI **excludes** the baseline (significant at p<0.05) â€” that loss is real, not noise.
...
Standard BM25+dense RRF k=60 â€” the textbook "lowest-regret" first move â€” **degrades nDCG@10 on both datasets** because our multi-field BM25 is weaker than Lucene's (our pure-BM25 NFCorpus = 0.279 vs Lucene 0.325). RRF averages BM25 noise into top-K when one input is materially weaker than the other.
...
| **RRF k=60 equal (default)** | 0.328 â†“ | 0.569 â†“ | **0.321 â†‘** | **0.951 â†‘** |
| RRF k=30 equal | 0.335 â†“ | 0.582 â†“ | 0.321 | 0.954 |
| RRF k=60 dense=1.2, bm25=0.8 | 0.334 â†“ | 0.577 â†“ | 0.324 | 0.961 |

Recall@100 **does** improve (RRF surfaces more candidates) â€” which makes RRF a useful *first stage* before reranking. Tracked for ADR-088 (cross-encoder rerank).

The default BEIR runner stays at dense-only. RRF is opt-in.

### ADR-088 â€” Lucene BM25 + cross-encoder rerank (3.10.28) â€” the pipeline that works
...
| ruflo Lucene BM25 alone | â€” | 0.505 | 0.328 | 0.681 |
| BM25 (published Lucene) | â€” | 0.502 | 0.325 | 0.679 |
...
### ADR-089 â€” 3-dataset BEIR (3.10.29)
...
**Rank 4 of 11 entries on the 3-dataset mean.** Beats published BM25 (+0.024), beats GTR-XL (with 1/10Ã— our params), beats Contriever, TAS-B, ColBERT, SBERT. Loses to SPLADE++ (-0.033), GenQ (-0.006, basically tied), and BGE-large (-0.088).

### Counter-findings honestly reported

**ArguAna kills the cross-encoder rerank.** Pulled at the 50-query checkpoint (running nDCG 0.283 vs dense alone 0.431). Estimated 6+ hours wall time and was actively hurting. ArguAna is counter-argument retrieval â€” rerank's pointwise relevance scoring doesn't help when the task requires understanding opposition.

**BGE-large NFCorpus = no lift.** Xenova/bge-large-en-v1.5 (335M, int8 quantized) measured 0.350 vs our BGE-base 0.352 â€” no improvement. Below the published BAAI BGE-large baseline (0.380). Likely Xenova int8 quantization + no query prefix. ADR-089.

**BGE query prefix is mixed.** Per BAAI's docs (`Represent this sentence for searching relevant passages: `): NFCorpus +0.009 âœ“, SciFact -0.007 âœ—, ArguAna +0.003 ~noise. Opt-in only via `BGE_QUERY_PREFIX=1`. ADR-090.

### ADR-091 â€” 4-dataset BEIR (3.10.30): SciDocs joins, dense alone wins it
...
| **4-dataset mean** | **0.421** | mixed | â€” |

### 4-dataset means â€” final leaderboard
...
**Rank 3 of 11 on 4-dataset mean.** Beats every published baseline except SPLADE++ (-0.012) and BGE-large (-0.070, mostly the ArguAna gap). Using a 110M-param base â€” beats GTR-XL's 1.2B (+0.016, 1/10Ã— the params).

### Third config-specific finding (SciDocs adds to the pattern)

| Dataset | Best config | What hurts |
|---|---|---|
| NFCorpus | Lucene+RRF+CE | nothing â€” full pipeline wins |
...
| **SciDocs** | **dense alone** | **RRF hurt by 0.008 (0.211 â†’ 0.203)** |
...
> BGE dense path** â€” no fine-tuning, no hybrid BM25+dense fusion, no
...
| BM25 (Lucene) | â€” | 0.325 | Thakur et al. 2021 |
| TAS-B | 66M | 0.319 | HofstÃ¤tter et al. 2021 |
...
| TAS-B | 0.643 | HofstÃ¤tter et al. 2021 |
...
int8-quantised ONNX. GPU would be ~10-50Ã— faster.

## Methodology notes

- **No fine-tuning.** All numbers are zero-shot â€” we use BAAI's released
...
- **`@xenova/transformers` direct API** (not `pipeline()`) used to bypass
  ...
  darwin-arm64 (ADR-085 Â§"sharp-on-darwin-arm64 bug").
- **CLS-token pooling + L2 normalisation** per BAAI's BGE spec; cosine
  becomes dot product on normalised vectors.
- **Graded relevance for nDCG** â€” qrels use 0/1/2 grades; we use
