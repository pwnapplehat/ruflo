# Ruflo â€” Overview Â· Usage Â· Status

> The complementary doc to [`USERGUIDE.md`](USERGUIDE.md) (deep reference) and [`/verification.md`](../verification.md) (cryptographic witness). This doc tells you **what Ruflo is**, **how to use it day-to-day**, and **what currently works** â€” without the encyclopedic reference depth.
...
- **MCP server** â€” exposes 323 tools to Cursor (memory, agents, swarm coordination, hooks, GitHub integration, browser automation, etc.).
- **CLI** â€” 45 top-level commands (`ruflo agent`, `ruflo swarm`, `ruflo memory`, `ruflo hooks`, `ruflo verify`, â€¦) for terminal/script use.
- **Cursor plugins** â€” 32 installable plugins (`ruflo-core`, `ruflo-federation`, `ruflo-cost-tracker`, â€¦) that bundle agent + skill + slash-command definitions.
- **WASM kernels** â€” Rust-compiled WASM for the policy engine, embeddings, and proof system; plugged into the same MCP/CLI surface.

For the "why" â€” coordinated swarms, self-learning memory, federated comms, enterprise security â€” see [`README.md`](../README.md).
...
2. **Just use Cursor normally**. Hooks automatically route tasks, retrieve relevant memory patterns, and coordinate background agents. You don't have to learn the 323 MCP tools â€” the routing layer does.

3. **Run the CLI for orchestration tasks** that don't fit naturally into Claude Code:
   - `ruflo agent spawn -t coder --name api-worker` â€” long-running agent
   - `ruflo swarm init --topology hierarchical --max-agents 8` â€” coordinated team
   - `ruflo memory search --query "auth patterns"` â€” semantic search across stored knowledge
   - `ruflo doctor --fix` â€” diagnose & repair install
   - `ruflo verify` â€” confirm your installed bytes match the signed witness
...
**Audit hardening â€” `audit_1776853149979`**:
...
**Encryption at rest â€” [ADR-096](../v3/docs/adr/ADR-096-encryption-at-rest.md), all 4 phases shipped**:
...
**Federation budget circuit breaker â€” [ADR-097](../v3/docs/adr/ADR-097-federation-budget-circuit-breaker.md), Phase 1 shipped**:
...
- Constant-string error reasons (`HOP_LIMIT_EXCEEDED`, `BUDGET_EXCEEDED`, `INVALID_BUDGET`) â€” no oracle leak.
...
| Track | Status |
|---|---|
| ADR-096 Phase 5 â€” `ruflo doctor` encryption status | pending |
| ADR-096 Phase 5+ â€” keychain (`keytar`) + passphrase resolvers | deferred |
| ADR-097 Phase 2 â€” peer state machine (ACTIVE / SUSPENDED / EVICTED) | deferred |
| ADR-097 Phase 3 â€” `ruflo-cost-tracker` integration | deferred |
| ADR-097 Phase 4 â€” `ruflo doctor` peer state + `federation_breaker_status` MCP tool | deferred |
...
Per-capability witness signing for the full 300-tool / 49-command surface is in flight â€” see tasks #25 / #26.
...
| Architecture decisions | [`v3/docs/adr/`](../v3/docs/adr/) â€” ADR-093, ADR-095, ADR-096, ADR-097 are the recent ones |
| Cryptographic proof of build correctness | [`verification.md`](../verification.md) + [`ruflo verify`](#verification) |
| Plugin development | [`USERGUIDE.md` â†’ Plugin section](USERGUIDE.md#-ecosystem--integrations) |
