#!/usr/bin/env node
/**
 * Regression guard for pwnapplehat/ruflo#1863 (and the crash class it represents).
 *
 ...
 * (exit code 1 *with* a TypeError/ReferenceError stack â€” graceful "not
...
if (!existsSync(CLI)) {
  console.error(`FAIL: ${CLI} not found â€” run \`npm --prefix v3/@claude-flow/cli run build\` first`);
  process.exit(1);
}
...
const watchdog = setTimeout(() => {
  console.log(`\n[watchdog] cli-no-crash exceeded 90s â€” exiting clean (CI safety). Checks completed so far stand.`);
...
function runCli(args, label) {
  ...
  if (r.error && r.error.code === 'ETIMEDOUT') {
    console.log(`note: ${label} â€” timed out after 30s (env-dependent backend); not failing on this`);
    return;
  }
  ...
  if (CRASH_RE.test(out)) {
    fail(`${label} â€” command crashed with an unhandled exception:\n${out.split('\n').filter(l => CRASH_RE.test(l)).slice(0, 3).join('\n')}`);
  } else {
    pass(`${label} â€” no unhandled exception (exit ${r.status})`);
  }
}

// --- #1863 â€” task create then task status must not crash on a fresh task ---
...
if (CRASH_RE.test(createOut)) {
  fail(`#1863-create â€” \`task create\` crashed:\n${createOut.split('\n').filter(l => CRASH_RE.test(l)).slice(0, 3).join('\n')}`);
} else {
  pass(`#1863-create â€” \`task create\` did not crash (exit ${createRes.status})`);
  ...
  if (idMatch) {
    runCli(['task', 'status', idMatch[0]], `#1863-status â€” \`task status ${idMatch[0]}\``);
  } else {
    // No ID extracted â€” fall back to `task list` which also formats the same arrays.
    runCli(['task', 'list'], '#1863-list â€” `task list` (formats the same arrays)');
  }
}
...
if (failed > 0) {
  console.error(`\n${failed} CLI command(s) crashed with an unhandled exception â€” see above. (#1863 class)`);
  process.exit(1);
}
