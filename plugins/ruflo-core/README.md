...
- **MCP Server**: 314 tools via `@claude-flow/cli` (memory, agentdb, embeddings, hooks, neural, autopilot, browser, aidefence, agent, swarm, system, terminal, github, daa, coordination, performance, workflow, â€¦)
...
- **Hooks**: PreToolUse / PostToolUse / PreCompact / Stop wired to claude-flow's auto-routing + learning loop. Defined at `plugins/ruflo-core/hooks/hooks.json` so the per-plugin loader picks them up on `/plugin install ruflo-core@ruflo` (per-plugin layout â€” fixes #1748 Issue 1; the marketplace-root copy at `.claude-plugin/hooks/hooks.json` is preserved for `claude --plugin-dir <repo-root>` users).
...
| `embeddings_*` | 10 tools incl. RaBitQ 32Ã— quantization | `ruflo-agentdb`, `ruflo-ruvector` |
| `hooks_*` (incl. `hooks_intelligence_*`) | 19+ tools â€” routing, learning, transfer, metrics, explain | `ruflo-intelligence`, `ruflo-autopilot` |
| `aidefence_*` | 6 tools â€” PII / prompt-injection / sanitization | `ruflo-aidefence` |
| `neural_*` | 6 tools â€” train, predict, patterns, compress | `ruflo-intelligence` |
| `autopilot_*` | 10 tools â€” autonomous loops + learning | `ruflo-autopilot` |
| `browser_*` (+ new `browser_session_*`) | 23 + 5 = 28 tools â€” Playwright + RVF lifecycle | `ruflo-browser` |
| `ruvllm_sona_*` / `ruvllm_microlora_*` | 4 tools â€” adaptive learning | `ruflo-intelligence`, `ruflo-ruvllm` |
...
| **4-step intelligence pipeline** (RETRIEVE â†’ JUDGE â†’ DISTILL â†’ CONSOLIDATE) | [ruflo-intelligence ADR-0001](../ruflo-intelligence/docs/adrs/0001-intelligence-surface-completeness.md) |
...
- [`ADR-0001` â€” ruflo-core plugin contract (foundation, MCP server, plugin catalog, smoke as contract)](./docs/adrs/0001-core-contract.md)
