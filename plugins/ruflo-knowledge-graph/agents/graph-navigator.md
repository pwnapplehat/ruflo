...
- `mcp__claude-flow__agentdb_semantic-route` -- semantic similarity routing for graph search (note: `semanticRouter` controller is `enabled: false` in current AgentDB builds â€” fall back to `agentdb_pattern-search` or `embeddings_generate` + manual cosine; see pwnapplehat/ruflo#2049)
