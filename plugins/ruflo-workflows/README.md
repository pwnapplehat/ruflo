...
1. **MCP `workflow_*` tools** â€” declarative, persisted workflow definitions with a full state-machine lifecycle (create â†’ run â†” pause â†’ complete/cancel). Best for long-lived, resumable, human-gated pipelines.
2. **Native Claude Code `Workflow` JS** â€” imperative orchestration scripts (`.claude/workflows/*.js`) that fan subagents out deterministically via `agent` / `parallel` / `pipeline` / `phase`. Best for comprehensive fan-out: review, audit, migration, research.

Neither subsumes the other â€” see [Choosing a surface](#choosing-a-surface).
...
The native surface runs a JavaScript orchestration script that fans subagents out deterministically. Scripts live in **`.claude/workflows/*.js`** and each begins with a **pure-literal** `export const meta` block â€” the `meta.name` makes it an invocable **named workflow**.
...
// body runs inside an async wrapper â€” top-level await + return are legal
...
| `pipeline(items, ...stages)` | Run each item through all stages independently â€” **no barrier** |
...
Workflow({ scriptPath, resumeFromRunId: 'wf_â€¦' })      // resume â€” unchanged agent() calls return cached
```
...
| A stateful lifecycle (`created â†’ running â†” paused â†’ completed/cancelled`) the engine schedules | **MCP `workflow_*`** |
...
This plugin owns the `workflows-state` AgentDB namespace (kebab-case, follows the convention from [ruflo-agentdb ADR-0001 Â§"Namespace convention"](../ruflo-agentdb/docs/adrs/0001-agentdb-optimization.md)). Reserved namespaces (`pattern`, `claude-memories`, `default`) MUST NOT be shadowed.
...
- [`ADR-0002` â€” native Claude Code Workflow orchestration (`.claude/workflows/*.js` fan-out) alongside the MCP surface](./docs/adrs/0002-native-workflow-orchestration.md)
