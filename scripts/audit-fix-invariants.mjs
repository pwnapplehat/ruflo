#!/usr/bin/env node
/**
 * Fix-invariants audit â€” guards against silent regression of the recent
 ...
 * fix â€” if a refactor accidentally removes it, the bug returns silently
 ...
 * @property {string} issue        â€” e.g. '#1945' (for failure messages)
 * @property {string} file         â€” repo-relative path
 * @property {string} [substring]  â€” must appear verbatim
 * @property {RegExp} [regex]      â€” must match somewhere in the file
 * @property {string} why          â€” what breaks if this line is removed
 */
/** @type {Invariant[]} */
const INVARIANTS = [
  // #1939 â€” Win32 cwd â†’ Claude Code slug
  {
    issue: '#1939',
    file: 'v3/@claude-flow/cli/src/mcp-tools/memory-tools.ts',
    regex: /\/\^\[A-Za-z\]:\[\\\\\/\]\//,
    why: 'Win32 slug candidate regex (`^[A-Za-z]:[\\/]`) â€” without it, memory_import_claude({allProjects:false}) returns 0 on Win32 paths like `C:\\Users\\â€¦\\Claude Stuff`.',
  },
  {
    issue: '#1939',
    file: 'v3/@claude-flow/cli/src/mcp-tools/memory-tools.ts',
    substring: "replace(/[:\\\\/]/g, '-')",
    why: 'Win32 slug normalization â€” drops `:` / `\\` / `/` so `C:\\Users\\â€¦\\Claude Stuff` â†’ `C--Users-â€¦-Claude-Stuff`.',
  },

  // #1941 â€” provision per-namespace vector_indexes row before entry insert
  {
    issue: '#1941',
    file: 'v3/@claude-flow/cli/src/memory/memory-bridge.ts',
    substring: 'INSERT OR IGNORE INTO vector_indexes (id, name, dimensions)',
    why: 'Per-namespace vector_indexes provisioning in bridgeStoreEntry â€” without it, memory_search({namespace:"X"}) returns 0 for any non-default namespace.',
  },
  {
    issue: '#1941',
    file: 'v3/@claude-flow/cli/src/memory/memory-initializer.ts',
    substring: 'INSERT OR IGNORE INTO vector_indexes (id, name, dimensions)',
    why: 'Per-namespace vector_indexes provisioning in storeEntry (sql.js fallback) â€” same root cause as the bridge path; one needs both branches.',
  },

  // #1943 â€” settings-generator project-local OR $HOME probe
  {
    issue: '#1943',
    file: 'v3/@claude-flow/cli/src/init/settings-generator.ts',
    substring: '[ -f "$D/',
    why: 'POSIX sh probe in hookCmd() â€” without it, global-install hook paths anchor at `${CLAUDE_PROJECT_DIR}` only and every Bash/Edit/Session hook fires MODULE_NOT_FOUND.',
  },
  {
    issue: '#1943',
    file: 'v3/@claude-flow/cli/src/init/settings-generator.ts',
    substring: 'IF EXIST',
    why: 'Windows `cmd /c IF EXIST â€¦ ELSE â€¦` fallback â€” Win32 equivalent of the sh probe.',
  },

  // #1945 / #1946 â€” memory bridge + doctor honor CLAUDE_FLOW_MEMORY_PATH
  {
    issue: '#1945',
    file: 'v3/@claude-flow/cli/src/memory/memory-bridge.ts',
    substring: 'getMemoryRoot',
    why: 'getDbPath() routes through getMemoryRoot() â€” without it, the bridge hard-codes `<cwd>/.swarm/memory.db` and CLI store writes to a different file than memory init created.',
  },
  {
    issue: '#1946',
    file: 'v3/@claude-flow/cli/src/commands/doctor.ts',
    substring: 'getMemoryRoot',
    why: 'doctor.checkMemoryDatabase() routes through getMemoryRoot() â€” without it, doctor reports "Not initialized" on any DB at a non-default path.',
  },

  // #1951 â€” statusline reads installed version from plugin package.json
  {
    issue: '#1951',
    file: 'v3/@claude-flow/cli/.claude/helpers/statusline.cjs',
    substring: 'RUFLO_VERSION',
    why: 'Startup version-probe variable in the deployed statusline â€” without it the header reverts to a hard-coded `RuFlo V3.5`.',
  },
  {
    issue: '#1951',
    file: 'v3/@claude-flow/cli/.claude/helpers/statusline.cjs',
    // Regex matches the path.join(home, '.claude', 'plugins', 'marketplaces', 'ruflo', â€¦) form.
    ...
    why: 'Plugin-install candidate path probed first â€” without it, plugin users always fall through to the hardcoded default.',
  },
  {
    issue: '#1951',
  ...
  // #1953 â€” hooks_pretrain code-file budget + code-dir-first traversal
  {
    issue: '#1953',
    file: 'v3/@claude-flow/cli/src/mcp-tools/hooks-tools.ts',
    substring: 'codeFilesScanned',
    why: 'Separate code-file budget counter â€” without it, the 50-file budget is burned by .md/.yaml/.db files and patternsExtracted: 0 on docs-heavy repos.',
  },

  // #1968 â€” daemon launcher forwards --workers / --headless / --sandbox
  {
    issue: '#1968',
    file: 'v3/@claude-flow/cli/src/commands/daemon.ts',
    regex: /forkArgs\.push\(['"]--workers['"]/,
    why: 'Daemon launcher forwards --workers to the forked child â€” without it, `daemon start --workers map` silently uses the default worker set.',
  },
  {
    issue: '#1968',
    file: 'v3/@claude-flow/cli/src/commands/daemon.ts',
    regex: /forkArgs\.push\(['"]--headless['"]\)/,
    why: 'Daemon launcher forwards --headless â€” same family as the --workers gap.',
  },

  // #1989 â€” statusline previously guarded raw SQLite header reads against
  // RFE1-encrypted memory.db (the bug rendered 3.3B `patterns` and cascaded
  // into fake DDD 5/5 / 100% / ðŸ§  100%). Superseded by #2196's delegation
  // refactor: statusline-generator.ts no longer reads .swarm/memory.db
  // bytes directly â€” it delegates to `hooks-statusline --json`, which
  ...
  // magic-check would ever be relevant. See pwnapplehat/ruflo#2216 for the
  ...
  // #1987 â€” memory stats uses persistent HNSW count from MCP tool, not
  ...
  // #1948 â€” Windows-specific statusLine command (no `sh` required)
  {
    issue: '#1948',
    file: 'v3/@claude-flow/cli/src/init/settings-generator.ts',
    regex: /process\.platform === ['"]win32['"]/,
    why: 'Platform-aware statusLine emission. On native Windows we emit `node -e "â€¦"` instead of `sh -c â€¦` so missing/mangled-quoting `sh` no longer produces stray repo-root files.',
  },

  // #1937 â€” exclusion patterns for memory_import_claude (voice-fidelity)
  {
    issue: '#1937',
    file: 'v3/@claude-flow/cli/src/mcp-tools/memory-tools.ts',
  ...
  // #1921 â€” pin @opentelemetry/core to dodge arborist Invalid Version
  {
    issue: '#1921',
    file: 'package.json',
    substring: '"@opentelemetry/core": "1.25.1"',
    why: 'overrides pin for @opentelemetry/core@1.25.1. Eliminates the npm 10.8.x arborist `Invalid Version: ` placeholder that fails every `npx claude-flow@alpha â€¦` install (including all 5 hook fires).',
  },

  // #1910 â€” MCP stdio mode protects stdout from stray console.log
  {
    issue: '#1910',
    file: 'v3/@claude-flow/cli/src/mcp-server.ts',
    substring: 'process.env.MCP_STDIO_MODE',
    why: 'stdio MCP server hijacks console.log/info/debug â†’ stderr so lazy-loaded module diagnostics never corrupt the JSON-RPC stream. Without this, hooks_route lazy-loads transformers.js/ONNX whose init prints to stdout and closes the Codex transport mid-batch.',
  },
  {
    issue: '#1910',
    file: 'v3/@claude-flow/cli/src/mcp-server.ts',
  ...
  // #1872 â€” SwarmCoordinator scaleAgents targets total, executeTask catches throws
  {
    issue: '#1872',
    file: 'v3/src/coordination/application/SwarmCoordinator.ts',
    substring: 'TARGET TOTAL',
    why: 'scaleAgents({type, count}) treats count as the target total, not a delta. Without this, repeated scaleAgents calls accumulate (1 â†’ 4 â†’ 6 instead of 1 â†’ 4 â†’ 2).',
  },
  {
    issue: '#1872',
    file: 'v3/src/coordination/application/SwarmCoordinator.ts',
  ...
  // ADR-120 Step 2 â€” midstream-aware federation loader prefers
  ...
  // Issue #1949 â€” agentic-flow is an OPTIONAL peer dependency, not a
  // hard runtime dep. Hardened npm registries that block the deep
  // koa-router â†’ cookies@0.9.1 transitive chain (under agentic-flow ->
  ...
  // ADR-120 Step 3 â€” ruflo-federation-peer Rust crate composes the
  ...
  },
  {
    ...
    why: 'Step 3 native_gate must adapt aimds_core::SafetyGate to the peer-local SafetyGate trait â€” without this import the adapter degrades to a typed placeholder.',
  },
  {
    issue: 'ADR-120',
    file: 'v3/crates/ruflo-federation-peer/src/lib.rs',
...
console.log(`fix-invariants audit â€” ${INVARIANTS.length} invariants across ${new Set(INVARIANTS.map(i => i.file)).size} file(s)`);
if (offenders.length === 0) {
  console.log(`  âœ“ all invariants present`);
  process.exit(0);
}

console.error(`\n  âœ— ${offenders.length} missing invariant(s):`);
for (const o of offenders) {
  console.error(`\n    [${o.issue}] ${o.file}`);
  console.error(`      ${o.error}: ${o.substring ?? o.regex}`);
  console.error(`      why this matters: ${o.why}`);
}
console.error('\n  If the fix moved to a different file or got refactored, update the invariant in scripts/audit-fix-invariants.mjs to point at the new location â€” do NOT delete it without confirming the regression is impossible by another mechanism.');
process.exit(1);
