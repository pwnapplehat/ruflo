#!/usr/bin/env node
/**
 * Regression guard for pwnapplehat/ruflo#2017.
 *
 ...
 * field â€” `toolInput` (the object) instead of `toolInput.command` (the
 * string) â€” so `.toLowerCase()` threw TypeError on every Bash call, was
 ...
 *
 *   - dangerous command â†’ exit 1 + `[BLOCKED] Dangerous command detected:`
 *   - innocuous command â†’ exit 0 + `[OK] Command validated`
 *   - empty payload    â†’ exit 0 (no crash, no false positive)
 *   - non-string command field â†’ exit 0 (defensive String() wrap holds)
 *
...
const cases = [
  ...
  },
  {
    ...
    name: 'null command field â†’ no crash',
  ...
  },
  {
    ...
    name: '#2017 shape: snake_case tool_input.command dangerous â†’ BLOCKED',
    input: { tool_name: 'Bash', tool_input: { command: 'rm -rf / --no-preserve-root' } },
    expectExit: 1,
    expectStderrIncludes: '[BLOCKED]',
  },
  {
    name: 'fork-bomb signature â†’ BLOCKED',
    input: { tool_name: 'Bash', tool_input: { command: ':(){:|:&};:' } },
    expectExit: 1,
    expectStderrIncludes: '[BLOCKED]',
  },
  {
    name: 'format c: â†’ BLOCKED',
...
function runOne(handlerPath, c) {
  ...
  // is a real bug â€” the safety gate is no longer running its check.
  ...
  // directly â€” the form the published 3.6.30 actually printed before the
...
if (failed > 0) {
  console.error(`\n${failed} pre-bash smoke case(s) failed â€” regression of #2017`);
  process.exit(1);
}
