#!/usr/bin/env node
/**
 * Smoke test for pwnapplehat/ruflo#2132 Ã¢â‚¬â€ Windows end-to-end hook execution.
 *
 ...
 * the hook command was "/bin/bash -c '...'" Ã¢â‚¬â€ a binary that does not exist
 * on native Windows.
 *
 * This test:
...
const initResult = spawnSync(
  ...
  [CLI_BIN, 'init', '--yes', '--skip-prompts', '--no-install'],
  {
    ...
    encoding: 'utf8',
    // iter 131 Ã¢â‚¬â€ bumped from 180s to 300s (5 min). macos-latest in this
...
function validateCommand(cmd, label) {
  if (!cmd) {
    console.log(`  skip: ${label} Ã¢â‚¬â€ command not found`);
    return;
  }
...
if (postEditCmd) {
  ...
  const env = {
    ...
    // ruflo@latest` Ã¢â‚¬â€ that takes 30+s on a cold CI runner and exceeds
  ...
  // Should not crash Ã¢â‚¬â€ accept any non-126 exit. Exit 0 is preferred (hooks
