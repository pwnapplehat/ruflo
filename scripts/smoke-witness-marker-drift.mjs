#!/usr/bin/env node
/**
 * Regression guard for pwnapplehat/ruflo#2021.
 *
 * #2021 happened because a dep bump (`@claude-flow/memory` alpha.14 â†’ 16)
 ...
 *     gap, not an integrity gap â€” separate concern).
...
// Markers are platform-agnostic â€” verify any one of the three OS
// manifests. macOS by convention (smallest cron-skew likely, but
// any would do â€” the fix entries themselves are identical).
...
if (fixes.length === 0) {
  console.error(`smoke-witness-marker-drift: 0 fixes in ${manifestPath} â€” manifest empty?`);
  process.exit(1);
}
...
console.log(`  file-missing (skipped â€” see #1880): ${missingFile.length}`);
...
if (drift.length > 0) {
  ...
  for (const f of drift) {
    ...
    console.error(`         desc:   ${f.desc.slice(0, 100)}${f.desc.length > 100 ? 'â€¦' : ''}`);
  }
  console.error(
    ...
    `â€¦for each of macos, linux, windows. The regen re-signs the\n` +
