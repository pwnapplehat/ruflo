/**
 * Entity tagger â€” regex-based proper-noun / structured-token extractor.
 *
 * Used by the `hybridSearch` controller as a third RRF arm alongside the
 * dense (vector) and sparse (BM25/FTS5) signals (ADR-147, pwnapplehat/ruflo#2317).
 *
 ...
 * Example: querying "Alice OAuth tokens" â€” BM25 may rank a doc about
 * generic OAuth above one mentioning Alice specifically; entity arm
 * surfaces the Alice doc independently.
 *
 * P1 is regex-only â€” no NLP model dependency. Tags:
 *   - Emails: foo@bar.com
 *   - URLs: http(s)://...
 ...
 *
 * Deliberately conservative â€” false negatives are fine (the dense + sparse
...
export function extractEntities(text: string): string[] {
  ...
  // Emails â€” anchored on @ so we don't pick up arbitrary handles.
  for (const m of text.matchAll(/[\w.+-]+@[\w-]+(?:\.[\w-]+)+/g)) add(m[0]);

  // URLs â€” http(s) only. Stop at whitespace and common boundary chars.
  for (const m of text.matchAll(/https?:\/\/[^\s)>"'`]+/g)) add(m[0]);

  // File paths â€” two complementary patterns:
  //
  ...
  // phrase with the *opening* quote of the next â€” e.g. `"a" over "b"`
  // should NOT capture ` over ` from positions 9â€“16.
  for (const m of text.matchAll(/(?<!\w)"([^"]{2,})"(?!\w)/g)) add(m[1]);
  for (const m of text.matchAll(/(?<!\w)'([^']{2,})'(?!\w)/g)) add(m[1]);

  // Proper-noun 2+ grams â€” capitalized words with capitalized neighbours.
