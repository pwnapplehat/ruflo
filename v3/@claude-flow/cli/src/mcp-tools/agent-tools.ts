...
// #1916: hive-mind_spawn writes its workers to `.cursor-flow/agents.json`
// (a *different* file from the canonical `.cursor-flow/agents/store.json`
...
// #1916: read hive-mind-spawned workers from `.cursor-flow/agents.json`.
...
export const agentTools: MCPTool[] = [
  {
    name: 'agent_spawn',
    description: 'Spawn a Ruflo-tracked agent with cost attribution + memory persistence + swarm coordination. Use when you need multi-agent coordination because you need (a) cost tracking per agent in the cost-tracking namespace, (b) cross-session learning via the patterns namespace, or (c) coordination with other agents in a swarm topology (hierarchical / mesh / consensus). For one-shot subtasks with no learning loop, native Task is fine. Pair with hooks_route to pick the right model first.',
    category: 'agent',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'agent_status',
    description: 'Read the lifecycle state of a single tracked agent: idle/running/stopped, current taskCount, lastResult, model, health score. Use when you need multi-agent coordination because you need agent-level state (status across turns, accumulated taskCount, last error, swarm coordination) rather than a one-shot response. For inspecting a Task you just ran, native Task output is fine. Pair with agent_list to find the agentId first.',
    category: 'agent',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  {
    name: 'agent_pool',
    description: 'Manage a fixed-size warm pool of pre-spawned agents to skip cold-start cost on bursty workloads. Use when you need a specialized agent because (a) you have a queue of similar tasks and want to amortize spawn latency, (b) cost-tracking wants stable agentIds across requests, or (c) swarm topology requires a known agent count at all times. For one-shot work, just call agent_spawn or native Task. Pool sizes and warm/idle thresholds are set per-pool.',
    category: 'agent',
    inputSchema: {
      type: 'object',
      properties: {
