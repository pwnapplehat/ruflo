...
async function guardInput(agentId: string, input: string) {
  ...
  if (!result.safe) {
    ...
    console.warn(`Ã¢Å¡Â Ã¯Â¸Â [${agentId}] ${result.threats.length} threat(s) detected`);
  ...
  if (result.piiFound) {
    console.warn(`Ã¢Å¡Â Ã¯Â¸Â [${agentId}] PII detected in input`);
  }
...
if (consensus.consensus === 'threat') {
  console.log(`Ã°Å¸Å¡Â¨ Security consensus: THREAT (${(consensus.confidence * 100).toFixed(1)}% confidence)`);
