...
function getDataDir() {
  const localDir = path.join(process.cwd(), '.cursor-flow', 'sessions');
  ...
  switch (platform) {
    case 'win32':
      return path.join(process.env.APPDATA || homeDir, 'cursor-flow', 'sessions');
    case 'darwin':
      return path.join(homeDir, 'Library', 'Application Support', 'cursor-flow', 'sessions');
    default:
      return path.join(homeDir, '.cursor-flow', 'sessions');
  }
}
