 ...
 *
 * This version delegates to 'npx @cursor-flow/cli hooks statusline --json'
...
// #2337: resolve an already-installed @cursor-flow/cli (or ruflo) bin so we
// can invoke it directly via `node`. The previous version called
// `npx --yes @cursor-flow/cli@latest` on every uncached render, which forces
...
function resolveCliBin() {
  try {
    ...
    const candidates = [
      ...
      path.join(CWD, 'node_modules', '@cursor-flow', 'cli', 'bin', 'cli.js'),
      path.join(CWD, 'node_modules', 'ruflo', 'bin', 'cli.js'),
      path.join(CWD, 'v3', '@cursor-flow', 'cli', 'bin', 'cli.js'),
    ];
    try {
      ...
      for (const gm of globalModuleDirs) {
        candidates.push(
          ...
          path.join(gm, '@cursor-flow', 'cli', 'bin', 'cli.js'),
        );
      }
    } catch { /* ignore */ }
...
function getStatuslineData() {
  ...
  try {
    ...
    // Fall back to `npx --prefer-offline @cursor-flow/cli` (no @latest) only
    ...
    const cmd = cliBin
      ...
      : 'npx --prefer-offline @cursor-flow/cli hooks statusline --json 2>/dev/null';
    const raw = execSync(
      cmd,
...
function getLocalADRCount() {
  const adrDirs = [
    ...
    path.join(CWD, '.cursor-flow', 'adrs'),
  ];
  let total = 0;
...
function getPkgVersion() {
  ...
  try {
    ...
    const pkgPaths = [
      ...
      path.join(CWD, 'node_modules', '@cursor-flow', 'cli', 'package.json'),
      path.join(CWD, 'node_modules', 'ruflo', 'package.json'),
      path.join(CWD, 'v3', '@cursor-flow', 'cli', 'package.json'),
    ];
    ...
    try {
      ...
      for (const gm of globalModuleDirs) {
        pkgPaths.push(
          ...
          path.join(gm, '@cursor-flow', 'cli', 'package.json'),
        );
      }
    } catch { /* ignore */ }
