...
> Seed: `ruvnet` Â· Seed type: `username` Â· Depth: 2 Â· Truncated: false
...
A ruflo-internal "Dossier" repo also exists (33 stars) â€” visual planning for AI coding â€” distinct from this dossier output.
...
| `ruflo` | repo | TS, 39,201 â­, MIT, multi-agent platform | gh-api, WebSearch, github.com |
| `RuView` | repo | Rust, 51,576 â­, WiFi sensing | gh-api |
| `RuVector` | repo | Rust, 3,891 â­, MIT, GNN vector DB | gh-api, codebase |
| `agentic-flow` | repo | TS, 668 â­, low-cost model switcher | gh-api, WebSearch |
| `ruv-FANN` | repo | Rust, 347 â­, neural net library | gh-api |
| `SynthLang` | repo | Python, 253 â­, prompt language for LLMs | gh-api |
| `open-claude-code` | repo | JS, 242 â­, clean-room CC reimpl | gh-api |
| `rvm` | repo | Rust, 95 â­, agentic VM | gh-api |
| `Dossier` | repo | 33 â­, visual AI coding planner | gh-api |
| `Cognitum.One` | url | personal blog / company site | gh-api (profile.blog) |
| `RuLake` | repo | Rust, vector exec fabric on RuVector | gh-api |
| `obsidian-brain` | repo | TS, Obsidianâ†”RuVector bridge | gh-api |
...
- `obsidian-brain` (Obsidian plugin â†’ RuVector)
...
- **Round 1 expansion** (parallel, 1 batch): `gh api repos/pwnapplehat/ruflo`, `gh api repos/ruvnet/RuVector`, `gh api repos/ruvnet/agentic-flow`, `gh api users/ruvnet/repos?sort=stars`
...
- `RuView` star count (51,576) substantially exceeds `ruflo` (39,201) â€” surprising given ruflo is the headline project; worth verifying.
- "Cognitum.One" is listed as the personal blog but also appears as an org in `cognitum-claude-plugin` â€” relationship between rUv / Not a Bot / Cognitum.One could be expanded in a depth-3 run.
