# ADR-145 â€” Plugin Supply-Chain Integrity and Memory Namespace Governance

**Status**: Proposed
**Date**: 2026-06-02
**Issue**: [pwnapplehat/ruflo#2254](https://github.com/pwnapplehat/ruflo/issues/2254)
**Related**: ADR-004 (Plugin Architecture), ADR-006 (Unified Memory), ADR-015 (Unified Plugin System), ADR-131 (ToolOutputGuardrail â€” content layer), ADR-144 (Authorization Propagation â€” runtime layer)

## Context

ADR-131 closed the content boundary and ADR-144 closes the runtime action boundary. Both assume the *code that's about to run* is trustworthy in the first place. Two Grade A papers published Aprilâ€“May 2026 demonstrate that assumption is broken under current Ruflo defenses.

### Threat: plugin supply-chain attacks

**Semantic Compliance Hijacking â€” SCH** (arXiv:2605.14460, May 14, 2026, Grade A): Eliminates the recognizable payload entirely. Instead of injecting malicious code, SCH wraps malicious intent in natural-language "compliance rule" text within plugin descriptions. The LLM agent reads the rule and **generates the harmful code itself at runtime**. Benchmark: **77.67% confidentiality breach, 67.33% RCE success, 0.00% scanner detection**. Signature-based defenses are completely blind because there is no static payload to sign against.

**DDIPE** (arXiv:2604.03081, Apr 3, 2026, Grade A): Document-Driven Implicit Payload Execution embeds malicious logic in plugin documentation and configuration templates. Tested across 4 frameworks, 5 models, 1,070 adversarial skills covering 15 MITRE ATT&CK categories. **Bypass rate 11.6â€“33.5%; 2.5% of adversarial skills evade both detection and alignment systems.** Four CVEs were disclosed from this research.
...
**MINJA** (arXiv:2601.05504, Jan 2026, Grade A): Query-only memory injection achieves **95% success under idealized conditions; 28â€“38% on production-deployed strong models**. The realistic-production number is still unacceptable for a system where compromised memory steers future agent behavior.

**Plan Injection** (arXiv:2506.17318, Jun 2026, Grade A): Context-chained memory attacks achieve **3Ã— higher attack success** than prompt-based injection and **+17.7% exfiltration gain** by creating logical bridges between unrelated memory entries. The fix is **namespace isolation with explicit write grants** â€” agents cannot poison a namespace they have no business writing to.
...
**Stage 2 â€” Semantic intent scan** (blocks SCH):
...
- `v3/@claude-flow/cli/src/plugins/store/discovery.ts` â€” verification hook on install
- `v3/@claude-flow/cli/src/plugins/trust/trust-anchors.json` (new)

### Part B â€” Memory namespace write ACLs

Address governance primitives 1â€“3 from the Mnemonic Sovereignty taxonomy (write authorization, read authorization, update authorization). Primitives 4â€“9 (retention, decay, audit, etc.) deferred for a future ADR.

- Every agent spawn receives an explicit `writeNamespaces: string[]` grant.
- AgentDB enforces grants at the storage boundary â€” **not** by convention in the calling code.
...
- `v3/@claude-flow/memory/src/agent-db.ts` â€” grant enforcement
- `v3/@claude-flow/cli/src/agent/spawn.ts` â€” `writeNamespaces` parameter
...
| P3 | Memory namespace ACL primitives 1â€“3 in AgentDB | `@claude-flow/memory/src/namespaces/` |
...
**Pattern-matching SCH at content boundary (ADR-131 extension).** SCH attacks succeed against content screening because the malicious content *is* the description â€” there's no instruction-shaped hijack to match. Catching it requires semantic intent classification at install, before the description ever enters agent context.

**Per-plugin sandboxing instead of signing.** Process-level sandboxing buys defense-in-depth but doesn't solve SCH: the malicious behavior is generated by the host model, not by sandboxed plugin code. Signing addresses the trust question; sandboxing addresses the blast-radius question â€” both belong on the roadmap, but signing closes the more urgent gap.

**Skip Part B; rely on ADR-131 for memory.** ADR-131 catches read-side injection. Plan Injection (arXiv:2506.17318) shows that allowing arbitrary writes lets attackers stage payloads that look innocuous individually but compose into a hijack across multiple reads â€” content screening cannot catch that compositional pattern. Write authorization is the missing piece.
...
- Positions Ruflo's memory governance ahead of every 2026 competitor surveyed â€” none satisfy more than 4 of the 9 Mnemonic Sovereignty primitives.
...
- Semantic intent scan adds **~50â€“200 ms** to `plugins install` â€” acceptable at install time, would be unacceptable at runtime.
...
P1 lands with:
- Unit tests covering signature verification against the existing official-plugin keys (round-trip sign â†’ verify; tamper-flips fail).
...
- arXiv:2605.14460 â€” *Exploiting LLM Agent Supply Chains via Payload-less Skills* (SCH)
- arXiv:2604.03081 â€” *Supply-Chain Poisoning Attacks Against LLM Coding Agent Skill Ecosystems* (DDIPE)
- arXiv:2601.05504 â€” *Memory Poisoning Attack and Defense on Memory-Based LLM Agents* (MINJA)
- arXiv:2506.17318 â€” *Plan Injection: Context-Chained Memory Attacks*
- arXiv:2604.16548 â€” *A Survey on the Security of Long-Term Memory in LLM Agents: Toward Mnemonic Sovereignty*
