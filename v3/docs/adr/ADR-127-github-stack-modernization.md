# ADR-127 â€” GitHub Skills, Agents, and Init-Template Modernization: CI Guards, Supply-Chain Review, and Provenance
...
The class of failure â€” **silent success when the check was never run** â€” matches #2017 exactly: the pre-bash hook exited 0 on dangerous payloads not because the block was absent but because a TypeError was silently swallowed. Any future refactor of the GitHub agents faces the same swallow risk unless there is a behavioral smoke.

### Category C: No `tools:` restrictions on 12 of 13 agents

The `github-project-management` skill is the single artifact with `allowed-tools:` in its frontmatter. All other skills and all 13 agents omit the field entirely, meaning they run with the full tool namespace when materialized. This includes `WebFetch`, which can be pointed at any URL â€” including URLs embedded in issue bodies or PR descriptions.
...
### Phase 1 â€” Static-contract smokes and supply-chain extension (small; ships first)
...
For every `uses:` line, asserts the ref is either (a) SHA-pinned (`uses: owner/repo@<40-hex>`) or (b) listed in a new `actions` key added to `.github/supply-chain/allowed-deps.json`. First offense: list the violating file + line. Fails on any violation. Zero runtime deps â€” pure `readFileSync` + regex.

**`.github/supply-chain/allowed-deps.json`** â€” add:
```json
"actions": {
...
```yaml
# Phase 1 smokes â€” gate on .github surface changes
on:
  push:
    paths:
...
### Phase 3 â€” Action pin upgrade and deprecated-action smoke (small)
...
**Upgrade `@v3` â†’ `@v4`** in all affected files: 5 agent files (`release-manager.md`, `release-swarm.md`, `swarm-pr.md`, `workflow-automation.md`, `repo-architect.md` â€” plus their init-template copies) and both skill files (`github-release-management/SKILL.md`, `github-code-review/SKILL.md`). This is a mechanical find-and-replace; the smoke in Phase 1 will catch future regressions.

The `ruvnet/swarm-action@v1` reference in `github-project-management/SKILL.md` (line 976) is either SHA-pinned to the current HEAD of that repo or replaced with the equivalent inline `run:` step. A SHA pin is preferred â€” it satisfies the allow-list check without removing the reference.
...
### Phase 4 â€” Attribution gate and `last-updated` stamps (small)

**Init-template command files** â€” gate the `ðŸ¤– Generated with` footer on the `attribution` option that `settings-generator.ts` already exposes. This is implemented via a new `ATTRIBUTION_FOOTER` variable in `helpers-generator.ts` that is set to an empty string by default and to the bot-identity string when `options.attribution === true`. Each of the 19 command files references the variable rather than hardcoding the footer.

This follows the precedent from `settings-generator.ts` lines 55-60 (introduced in #2079 / #2078). The bot email `ruflo-bot@users.noreply.github.com` applies to commit trailers; the PR/issue body footer should use `ðŸ¤– Generated with [RuFlo](https://github.com/pwnapplehat/ruflo)` (same PR footer already in `settings-generator.ts:60`) when attribution is opted in, and no footer otherwise.

**`Last Updated` stamps** on the four skill SKILL.md files are updated from 2025-10-19 to the date this ADR ships. A comment in each file notes: "update this date when you change the skill so the smoke can detect stale copies." This is convention, not enforced â€” no smoke needed.

Acceptance: `ruflo init` (without `--attribution`) produces command files with no `ðŸ¤–` footer string in any PR/issue body template; `ruflo init --attribution` produces the `RuFlo` bot-identity footer.
...
**Init-template constraint**: anything added to `ruflo init` output must work in an empty directory with only `node` and `gh` available. All four phases satisfy this: static markdown files and `.js`/`.sh` helpers â€” no new runtime dependencies installed into user projects.
...
- `smoke-github-actions-pins.mjs` catches any future commit that copies old `@v3` snippets from blog posts into the skill files â€” a very common doc-drift pattern.
...
- Phase 4's attribution gate is only enforced at `init` time. A user who edits their materialized command files after `init` can re-add the footer manually. This is acceptable â€” the commitment is to the default (`init` output without `--attribution` contains no footer), not to preventing all possible edits.

### Neutral

- The `@v3` â†’ `@v4` pin upgrade is compatible with all runner images in `v3-ci.yml` (already running `@v4` for the main CI jobs). No runner image changes needed.
...
| 2 | Helper hardening + agent frontmatter + unquoted interpolation fix | `github-safe.js` Ã— 2, `github-setup.sh` Ã— 2, 13 agent files Ã— 2 (dogfood + init-template) | M | `smoke-github-safe-injection.mjs` passes hardened copies; grep for unquoted interpolation returns 0 matches |
| 3 | Deprecated-action smoke + `@v3` â†’ `@v4` pin upgrade | `scripts/smoke-github-release-no-deprecated-action.mjs` (new), 6 agent files Ã— 2, 2 skill files | S | Both new smokes pass; `smoke-github-actions-pins.mjs` passes all in-scope files |
| 4 | Attribution gate in init-template + skill `Last Updated` stamps | `v3/@claude-flow/cli/src/init/helpers-generator.ts`, 19 command files, 4 skill files | S | `ruflo init` without `--attribution` produces no `ðŸ¤–` footer; `ruflo init --attribution` produces bot-identity footer |

Recommended landing: Phases 1 + 3 in one PR (pure static checks + mechanical upgrades â€” no behavioral change). Phase 2 in its own PR (behavioral change to agents and helpers â€” needs explicit review). Phase 4 in its own PR (init-template behavior change).
...
6. `ruflo init` (no `--attribution`) produces 19 command files with no `ðŸ¤– Generated with` string in any PR/issue body template.
...
- **ADR-128**: Provenance / witness signing for `gh release create` artifacts â€” generalizing the ADR-103 witness pattern to the release pipeline. This ADR establishes that the helper surface must _not_ recommend deprecated Actions; it does not yet mandate Ed25519-signed release artifacts.
- **ADR-129**: GitHub Apps token scoping â€” a separate decision about whether `ruflo init`-materialized workflows should request `contents: read` by default rather than inheriting the ambient `GITHUB_TOKEN` scopes. Gated on a broader permissions-minimization ADR.
...
- **Dynamic action pinning automation** (`dependabot.yml` or Renovate configuration for `uses:` refs) â€” net-new pattern with no prior art in this repo. Phase 3 does the manual upgrade; automation is a follow-on.
