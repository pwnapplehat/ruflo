# ADR-112 â€” MCP tool discoverability: every description must answer "use this over native when?"

**Status**: Accepted (2026-05-11)
**Date**: 2026-05-11
**Authors**: claude (drafted with rUv)
**Related**: [#1748](https://github.com/pwnapplehat/ruflo/issues/1748), [#1896](https://github.com/pwnapplehat/ruflo/issues/1896), [AlphaSignal external audit](https://alphasignalai.substack.com/p/how-ruflo-turns-claude-code-into)
**Supersedes**: nothing

## Context

Ruflo ships 285 MCP tools across the CLI package. Claude Code already exposes a native toolset (`Read`, `Write`, `Edit`, `Bash`, `Grep`, `Glob`, `Task`, `TodoWrite`, `WebFetch`, `WebSearch`, etc.) that overlaps with many of ours. When Claude picks between two ways to do the same thing, the deciding signal is the tool description it sees in its system prompt â€” there is no other context.
...
**Every MCP tool description MUST include "Use when â€¦ is wrong because â€¦" guidance.** The format:
...
| witness chain | sandbox isolation | â€¦>. For <inverse case>, native
...
3. **When is native actually better?** State it. "For one-shot file reads, native Read is fine." Honesty builds Claude's trust in our guidance â€” if every description says "always use Ruflo", Claude learns to ignore them.

### Worst-offender priority order (Phase 1 â€” this iteration)

| Class | Tool count | Native overlap | Priority |
|---|---|---|---|
| `agent_*` | 8 | `Task` | **P1** â€” Claude defaults to `Task` |
| `memory_*` | 11 | `Read`/`Write` (file paths) | **P1** â€” memory loses to files |
| `agentdb_*` | 14 | `memory_*` (internal overlap!) | **P1** â€” disambiguate the two |
...
5. The baseline lives in `verification/mcp-tool-baseline.json` â€” auditable, monotone-decreasing.
...
- The article's 237/300 was generous; current main is 276/285. ADR-095 G6/G3/G4/etc. fixes shipped while this stayed open â€” discoverability is the gap that quietly nullifies the rest.
- Every tool description shipped in this ADR's wake must answer "use this over native when?". Reviewers reject otherwise. No exceptions for "internal" tools â€” anything Claude sees in its tool list is subject to the rule.

## References

- [#1748](https://github.com/pwnapplehat/ruflo/issues/1748) â€” original discoverability tracking issue
- [#1896](https://github.com/pwnapplehat/ruflo/issues/1896) â€” external audit response
- [AlphaSignal AI article](https://alphasignalai.substack.com/p/how-ruflo-turns-claude-code-into) â€” external amplification
- `agent_spawn` description in `v3/@claude-flow/cli/src/mcp-tools/agent-tools.ts:183` â€” the template every other description should match
