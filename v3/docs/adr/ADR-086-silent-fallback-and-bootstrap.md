# ADR-086 â€” Silent-Fallback Bug Story + Bootstrap Significance Testing

**Status**: Accepted â€” Implemented in ruflo 3.10.26
**Date**: 2026-05-30
**Tracking**: continuation of self-learning hardening cluster (ADR-077 â†’ ... â†’ 085)
...
1. **The silent-fallback embedder bug.** While building ADR-085's BEIR harness on darwin-arm64, every retrieval result looked plausible but cosine similarity scores were essentially random. The neural store reported `_realEmbedding: true`. The hash-fallback was carrying the entire dense signal â€” and we didn't know it. This is the kind of bug that *steals years of debugging* from people who don't notice.

2. **Statistical significance on the 0.005 SPLADE++ gap.** Our NFCorpus result (0.352) is 0.005 above SPLADE++ (0.347). The natural pushback is "noise" â€” and with N=323 queries it's a fair pushback. We need a paired bootstrap CI to either confirm the gap is real or admit it's not.
...
neural_patterns store action
  â†’ realEmbeddings.embed(text)
    â†’ agentic-flow/reasoningbank.computeEmbedding(text)
      â†’ @xenova/transformers pipeline('feature-extraction', 'all-MiniLM-L6-v2')
        â†’ @xenova/transformers loads tokenizer + sharp (for image models)
          â†’ require('sharp')
            â†’ require('../build/Release/sharp-darwin-arm64v8.node')
              â†’ MODULE_NOT_FOUND
            â†“ THROWS
          â†“ THROWS
        â†“ THROWS
      â†“ but the THROW happens per-CALL, not at import time
    â†“ neural-tools wraps each call in try/catch and silently hash-falls-back
  â†“ store returns successfully
â†“ neural_patterns reports _realEmbedding: true (because realEmbeddings is non-null at module-load)
```
...
downstream `transformers.js â†’ sharp` chain breaks â€” because the import is
...
- **Bypassed the sharp dependency** in ADR-085 by loading BGE directly
  ...
  don't need image preprocessing â€” `sharp` is a transitive dep of the
...
- **Documented in `BEIR-MATRIX.md`** and ADR-085 Â§"sharp-on-darwin-arm64
...
success â‰  runtime correctness. External benchmarks expose the gap because
they don't share the bias of internal labels.

### 2. Paired bootstrap significance test

`scripts/beir-bootstrap-significance.mjs` â€” given a run JSON with
...
- **CI overlap test vs each published baseline** â€” if our 95% CI lower
  bound exceeds the baseline, the difference is significant at p<0.05.
- **Paired bootstrap** (via `--paired <other-run.json>`) â€” resamples the
...
=== vs each published baseline (CI overlap) ===
  0.272  SBERT msmarco              Î”=+0.0797  â†‘ above   [p<0.05 â€” significant win]
  0.305  ColBERT                    Î”=+0.0467  â†‘ above   [p<0.05 â€” significant win]
  0.319  TAS-B                      Î”=+0.0327  â†‘ above   n.s.
  0.319  GenQ                       Î”=+0.0327  â†‘ above   n.s.
  0.325  BM25 (Lucene)              Î”=+0.0267  â†‘ above   n.s.
  0.328  DocT5query                 Î”=+0.0237  â†‘ above   n.s.
  0.328  Contriever                 Î”=+0.0237  â†‘ above   n.s.
  0.343  GTR-XL                     Î”=+0.0087  â†‘ above   n.s.
  0.347  SPLADE++                   Î”=+0.0047  â†‘ above   n.s.
  0.380  BGE-large-v1.5             Î”=-0.0283  â†“ below   n.s.
```

**Two significant wins** (SBERT, ColBERT). Seven point-estimate wins are within sampling noise (n.s.) â€” the "rank-2" headline is a single-realisation outcome, not a statistically distinguishable lead over SPLADE++/GTR-XL/BM25 etc. **The 0.005 SPLADE++ gap is noise**, as expected.
...
=== vs each published baseline (CI overlap) ===
  0.555  SBERT msmarco              Î”=+0.0706  â†‘ above   [p<0.05 â€” significant win]
  0.643  TAS-B                      Î”=-0.0174  â†“ below   n.s.
  0.644  GenQ                       Î”=-0.0184  â†“ below   n.s.
  0.662  GTR-XL                     Î”=-0.0364  â†“ below   n.s.
  0.671  ColBERT                    Î”=-0.0454  â†“ below   n.s.
  0.675  DocT5query                 Î”=-0.0494  â†“ below   [p<0.05 â€” significant loss]
  0.677  Contriever                 Î”=-0.0514  â†“ below   [p<0.05 â€” significant loss]
  0.679  BM25 (Lucene)              Î”=-0.0534  â†“ below   [p<0.05 â€” significant loss]
  0.704  SPLADE++                   Î”=-0.0784  â†“ below   [p<0.05 â€” significant loss]
  0.722  BGE-large-v1.5             Î”=-0.0964  â†“ below   [p<0.05 â€” significant loss]
```
...
**On the two-dataset mean, we lose to BM25** (0.489 vs 0.502). The NFCorpus rank-2 is real but not representative. The acceptance test "beats BM25 on both datasets" fails â€” and reporting that honestly is the point of this ADR.

## Honest limits

- **N=323 is the dataset's full test split** â€” we can't add more queries
  without leaving NFCorpus.
- **Bootstrap assumes per-query independence** â€” true for IID query sets,
...
- **Fix the `_realEmbedding: true` lie** at source â€” needs a probe-embed
  at module init. Tracked separately.
- **Lucene-style BM25 improvements** to close our 0.289 â†’ 0.325 BM25 gap.
