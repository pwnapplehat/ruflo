# ADR-074 â€” Self-Learning Wiring + Honest Multi-Path Output (#2245)

**Status**: Accepted â€” Implemented in ruflo 3.10.14
**Date**: 2026-05-30
**Tracking issue**: [#2245](https://github.com/pwnapplehat/ruflo/issues/2245)
**Companion**: ADR-143 (deterministic Tier-1 codemods â€” same claims-vs-reality pattern), ADR-073 (agentdb SOTA â€” also a claims-vs-reality round), [pwnapplehat/ruflo#2241](https://github.com/pwnapplehat/ruflo/issues/2241) Dream-Cycle research (Structured Distillation is the natural Round-2 enhancement to this work)
...
4. Persistence reset bug: `loadPersistedStats()` only restored `trajectoriesRecorded` â€” `patternsLearned` and `signalsProcessed` were reset to 0 on every process restart, masking real learning history.
...
The reporter correctly identified that the system has **one path that actually works**: `hooks_intelligence_trajectory-start â†’ -step â†’ -end`. Everything else is either a stub, a write to a different store than the dashboard reads from, or a permanently-zero counter.

This is the same family as ADR-143 (Agent Booster Tier-1 was dead/mislabeled) and ADR-073 Â§A (recall@10 was un-measured). A real engine exists; convenience surfaces advertise capabilities they never actually invoke.

## Decision

Ship ruflo 3.10.14 with the **three minimal wirings** plus **honest multi-path output**. The full unification (one source of truth for all 4 aggregators) is a multi-PR program tracked separately â€” not crammed into this round.

### Â§1 â€” Wire `hooks_task-completed` to the trajectory pipeline

When `trainPatterns: true`, synthesize a one-step trajectory from `{ taskId, success, quality, content }` and call the existing `recordTrajectory()` helper in `intelligence.ts` â€” the same code path `hooks_intelligence_trajectory-end` invokes. The handler now:
...
Also fix `loadPersistedStats()` to restore `patternsLearned` and `signalsProcessed` alongside `trajectoriesRecorded` â€” so a process restart no longer zeroes the learning history.

### Â§3 â€” `hooks_pretrain` writes per-pattern rows + honest output
...
- **N per-pattern rows** to the neural store via `storeNeuralPatterns` â€” so `neural_patterns list` actually returns them.
...
### Â§4 â€” Honest multi-path messaging
...
- The `note` strings tell callers what *didn't* happen, not just what did. ("recorded only â€” pass trainPatterns:true to feed the learning loop", not "[OK] Outcome recorded.")

### Â§5 â€” Adversarial input hardening (OWASP ASI06)
...
- **Unify the 4 stat sources** â€” designate `globalStats` as authoritative, derive `memory_bridge_status`, `hooks metrics`, and `neural_patterns count` from it. Multi-PR design work.
- **Wire `post-edit` / `post-command`** to feed the trajectory pipeline â€” design call (which store wins).
- **Structured distillation** of trajectory content (#2241 Â§Structured Distillation) â€” 11Ã— compression + +1.4 MRR. Natural Round-2 enhancement.
- **Schedule consolidation worker** â€” background NightlyLearner instead of on-demand only.
- **Wire MCP trajectory-end to globalStats** too â€” currently feeds `sonaCoordinator` only.
...
- `neural_patterns list` reflects what `pretrain` claims â€” no more "47 extracted, 0 listed" gap.
...
- `__tests__/self-learning-2245.test.ts` â€” 9 tests across EASY (primitives), MEDIUM (MCP surfaces), and COMPLEX (multi-step + persistence) categories. CI gate breaks if any of the three fixes regresses.
- `scripts/benchmark-self-learning.mjs` â€” proof harness with 5 sections (Aâ€“E), writes a committed run JSON. Reproduces with `N=20 node scripts/benchmark-self-learning.mjs` from a fresh checkout.
- Build clean (`tsc -b`); full CLI test suite still green.
- Manual end-to-end:
  - `recordSignalProcessed` Ã— 10 â†’ `signalsProcessed = 10` âœ…
  - `hooks_task-completed {trainPatterns:true}` Ã— 10 mixed-verdict â†’ 10 trained via trajectory pipeline, ~18 ms/call avg âœ…
  - `hooks_task-completed` (no trainPatterns) Ã— 10 â†’ trajectories+0 (correct negative control) âœ…
  - `storeNeuralPatterns` Ã— 10 â†’ `neural_patterns list` returns â‰¥10 âœ…
