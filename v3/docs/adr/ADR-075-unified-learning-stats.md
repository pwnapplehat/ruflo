# ADR-075 â€” Unified Learning Stats Aggregator (#2245 follow-up)

**Status**: Accepted â€” Implemented in ruflo 3.10.15
**Date**: 2026-05-30
**Tracking**: [#2245](https://github.com/pwnapplehat/ruflo/issues/2245) â€” self-learning subsystem audit
**Supersedes**: the "future-rounds" item in ADR-074 Â§"Deliberately NOT in this round" titled *"Unify the 4 stat sources"*
...
When we tried that, we found the premise was wrong: the four sources **genuinely measure different things**. They look like duplicates of one number because they all answer "did learning happen?" â€” but each store is the authoritative record for a different layer:
...
| `memory-bridge` (AgentDB) | "how many memory entries exist across all namespaces" â€” way broader than learning |
...
- `getMemoryBridgeStats({ namespaces? })` in `memory-bridge.ts` â€” read-only top-level + per-namespace entry counts. Best-effort: returns `reachable:false` instead of throwing when the bridge isn't loadable.
- `getNeuralStoreStats()` in `neural-tools.ts` â€” top-level + per-`type` pattern count from the neural store file.
...
- `sona.trajectoriesTotal` is expected to track `globalStats.trajectoriesRecorded` within Â±1 between a process restart and the next `clearIntelligence()`. Drift outside Â±2 is a `consistency.notes` entry.
- When `globalStats.patternsLearned > 0` but `neuralPatterns.patternCount === 0`, the consistency block reports it explicitly â€” that's the pretrain-vs-trajectory-vs-neural-store gap the reporter caught in #2245.
...
- Schema-level unification (one store, others derived). Wrong design â€” see Context.
...
- The `consistency.notes` array catches the specific drifts the reporter found (the "four contradictory sources" complaint) â€” they're now surfaced as flags on the unified call rather than as silent disagreement between separate dashboards.
...
- `__tests__/unified-stats-2245.test.ts` â€” 7 tests:
  - Shape: all four sub-views present with `source` fields âœ…
  - Driving each path moves the right counter:
    - `recordSignalProcessed` â†’ `global.signalsProcessed` up âœ…
    - `storeNeuralPatterns` â†’ `neuralPatterns.patternCount` up âœ…
    - `hooks_task-completed {trainPatterns:true}` â†’ `global.trajectoriesRecorded` up âœ…
  - Consistency block flags drift instead of staying silent âœ…
  - MCP tool registered + returns the unified shape âœ…
- `scripts/benchmark-self-learning.mjs Â§F` â€” proof artifact in `docs/benchmarks/runs/self-learning-latest.json`.
...
# 6-section benchmark including Â§F unified-stats
