# ADR-146 â€” ToolOutputGuardrail Integration Rollout (ADR-131 P2â€“P5)

**Status**: Proposed
**Date**: 2026-06-02
**Issue**: [pwnapplehat/ruflo#2149](https://github.com/pwnapplehat/ruflo/issues/2149) (follow-up â€” original ADR-131 closed P1 only)
**Related**: ADR-131 (ToolOutputGuardrail â€” P1 shipped), ADR-144 (Authorization Propagation), ADR-145 (Plugin Supply-Chain Integrity)

## Context

ADR-131 shipped Phase 1 only: the `ToolOutputGuardrail` class is exported from `@claude-flow/security`, has tests and OWASP mapping docs, and is callable. It is not yet wired into a single hot path. The phased plan in ADR-131 Â§Integration Plan named four more phases (P2â€“P5) that close the actual ASI01 gap by running the guardrail at every place content crosses the agent boundary. None have shipped.

A status-quo system with the class but no call sites is worse than not having the ADR at all â€” it implies coverage we don't have. Issue #2149 was filed against the ASI01 gap and was left open because P1 shipped without P2â€“P5; closing it requires running the guardrail at the four content-entry boundaries it was designed for.

### What still leaks today

| Content path | Bypasses guardrail? | Attack surface |
|---|---|---|
| MCP tool result â†’ agent context | **Yes** â€” no call site in dispatcher | Any compromised MCP server response (40.55% unauthenticated per ADR-144 evidence) |
| Memory read (`memory_search`, `memory_retrieve`) â†’ agent context | **Yes** â€” no call site in retrieve path | Memory-poisoning chains (MINJA / Plan Injection per ADR-145 evidence) |
| Raft consensus state-transition payload â†’ swarm | **Yes** â€” no call site in proposal pipeline | SwarmRaft-class injection (covered as "swarm-layer ASI01" in #2149) |
| Hooks tool output (third-party plugins) | **Yes** â€” no policy plumbing | Any plugin tool that doesn't run its own screen |

P1's class works in isolation but nothing in the runtime calls it. The shape of the gap is identical whether the attack vector is an indirect-injection paper from January or a brand-new one â€” boundaries with no screening are boundaries with no defence.

### Why a follow-up ADR (instead of just opening four PRs)

P2â€“P5 collectively add a new policy layer to four independent hot paths. Each path needs a per-boundary decision on `flag` / `redact` / `reject` defaults, an observability contract for the resulting telemetry, and a backwards-compatibility story (the swarm-layer integration in particular cannot break consensus). Those decisions are architectural enough that they should be recorded once, here, instead of relitigated in each PR review.
...
**Policy defaults**: `low â†’ allow`, `medium â†’ flag (log only)`, `high â†’ redact`, `critical â†’ reject`. Rejection surfaces to the calling agent as a typed tool error â€” never silently drops to empty content (this rule was in ADR-131 P1 but is restated here so callers can rely on it).

### P3 â€” Memory read path
...
A reject finding here returns the entry with `content: <removed-by-guardrail>` and a structured warning rather than excluding the entry entirely â€” the caller needs to know the entry exists to track namespace state.

### P4 â€” Raft consensus payload validator

**Where**: hive-mind proposal pipeline (`v3/@claude-flow/cli/src/hive-mind/*` â€” exact file pinned during P4 PR).
...
### P5 â€” Per-tool policy overrides + structured telemetry
...
interface GuardrailEvent {
  ...
  result: GuardrailResult;          // { severity, category, pattern, ... } â€” exact shape from ADR-131
...
**Ship P2 first, defer P3â€“P5 indefinitely.** P2 alone closes ~60% of the ASI01 attack surface (MCP tool results are the most common content boundary). But MINJA / Plan Injection specifically target the memory read path P2 doesn't cover; partial rollout misadvertises coverage.

**Replace P4 with a separate consensus-layer ADR.** Defensible â€” the consensus interaction is the highest-complexity phase â€” but creates a four-ADR situation for what is fundamentally one rollout. The split makes more sense if a P4-specific design problem surfaces during P3 telemetry analysis; the current decision is to keep it under ADR-146 unless that happens.
...
- Each phase adds latency. ADR-131 P1's class is pattern-based and sub-millisecond; the new call sites should preserve that, but P3's read path executes on every memory hit â€” needs to be benchmarked at the same time the call is added, not after the fact.
- More events on the security telemetry channel â€” needs a sampling story for high-volume deployments. Default sampling rate at P5: `flag` 10%, `redact`/`reject` 100%.

**Deferred**:
- Model-based classifier as an out-of-band reviewer (mentioned but deferred in ADR-131) â€” same deferral here. Pattern-based remains the only thing on the hot path.
- Non-English pattern sets â€” community contribution pathway; not blocking P2â€“P5.
