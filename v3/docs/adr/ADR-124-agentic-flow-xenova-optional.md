# ADR-124 â€” Upstream `agentic-flow` fix: move `@xenova/transformers` to optionalDependencies

**Status**: Proposed (2026-05-19)
**Date**: 2026-05-19
**Authors**: claude (drafted with rUv)
**Related**: [`agentic-flow@2.0.11`](https://www.npmjs.com/package/agentic-flow), [ruvnet/agentic-flow](https://github.com/ruvnet/agentic-flow), ADR-118 (AIDefence 2.3.0), ADR-121 (embeddings RuVector upgrade â€” long-term Xenova migration), ADR-122 (browser substrate), supply-chain hardening PR #2050, ruflo issue #2046
...
ruflo-browser-consumer
  â””â”€â”€ @claude-flow/browser
        â””â”€â”€ agentic-flow ^2.0.11
              â”œâ”€â”€ @xenova/transformers ^2.17.2  â† HIGH CVE via onnxruntime-web â†’ protobufjs
              â””â”€â”€ agentdb (opt) ^3.0.0-alpha.14
                    â””â”€â”€ @opentelemetry/sdk-node (opt) ^0.52.0  â† HIGH CVE, fixed in our root overrides
```

The `agentdb â†’ @opentelemetry/sdk-node` chain was fixed in [PR #2050 commit `9a8c9c464`](https://github.com/pwnapplehat/ruflo/commit/9a8c9c464) by bumping agentdb to `3.0.0-alpha.14` + npm overrides pinning `@opentelemetry/sdk-node â‰¥ 0.218.0`. Overrides do NOT cascade across separate npm projects, so the same block was duplicated in `v3/@claude-flow/browser/package.json` to keep standalone installs clean.

What remains: **`@xenova/transformers` is a direct dependency of `agentic-flow`**, and `agentic-flow` is a direct dependency of `@claude-flow/browser`. Overrides on the consumer side can pin to a newer Xenova version â€” but the latest is 2.17.2, and the only fix-available is `@xenova/transformers@2.0.1` (a *downgrade* via a major version-bump). Upgrading would break Xenova's runtime API in ways the agentic-flow embedding code paths aren't prepared for, and downgrading to 2.0.1 sheds three years of ONNX-runtime improvements.

The **right fix lives upstream** in `agentic-flow` itself.

## What we found in the upstream source

`/Users/cohen/Projects/agentic-flow/agentic-flow/src/` â€” the published package's source. Six call sites import `@xenova/transformers`. Five of them already use **dynamic import** (`await import('@xenova/transformers')`):
...
Five-out-of-six already gracefully degrade when the module is absent. The only file that doesn't is `src/reasoningbank/utils/embeddings.ts` â€” a static import at the top of the file. That's the single change we need to make upstream to qualify the entire package for moving Xenova to `optionalDependencies`.
...
3. **Bump version to `2.0.12`** (patch â€” behavior is preserved for consumers that have `@xenova/transformers` installed; the change is purely about *who decides to install it*).
...
- `agentic-flow` test suite passes (especially `src/reasoningbank/` tests â€” that's the file we touched)
...
1. Bump `v3/@claude-flow/browser/package.json` â†’ `agentic-flow ^2.0.12`
...
4. Remove the `cve[]` entry for `agentic-flow â†’ @xenova/transformers` from `.github/supply-chain/accepted-findings.json`
5. Re-run `node scripts/audit-supply-chain.mjs` â€” should pass with 0 unaccepted findings AND no accepted entries
6. Open a PR amending PR #2050 (or follow-on PR) titled `fix: bump agentic-flow to 2.0.12 (closes the last accepted CVE)`

## Out of scope

- **Full Xenova retirement** â€” ADR-121 Phase 4 tracks the larger migration to `ruvector-onnx-embeddings-wasm`. This ADR is a tactical patch that lets ruflo ship a clean supply-chain audit *now*; the strategic migration continues independently.
- **Bumping `@xenova/transformers` itself to v3** â€” v3 was renamed to `@huggingface/transformers`. That migration involves API changes and is what ADR-121 Phase 4 addresses. Not in scope here.
- **Touching `@claude-flow/embeddings`** â€” also covered by ADR-121.
