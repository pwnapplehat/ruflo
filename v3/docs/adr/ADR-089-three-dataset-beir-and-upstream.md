# ADR-089 â€” 3-Dataset BEIR + Upstream ruvector + User Bug Fixes

**Status**: Accepted â€” Implemented in ruflo 3.10.29
**Date**: 2026-05-30
...
1. **3rd BEIR dataset (ArguAna)** â€” strengthens the 2-dataset story to a 3-dataset story.
2. **BGE-large NFCorpus ceiling test** â€” answered: no lift on this hardware.
3. **ruvector@0.2.27 Tier-0 wiring** â€” kills the silent-fallback bug (ADR-086) at source.
4. **4 user-reported bugs from #2246** â€” 3 fixed in ruflo, 1 forwarded to ruvnet/agentdb#7.
...
| **3-dataset mean** | **0.491** | mixed | â€” | BGE-large 0.579 |
...
| BM25 (published Lucene) | â€” | 0.325 | 0.679 | 0.397 | **0.467** |
| ruflo Lucene BM25 alone | â€” | 0.328 | 0.681 | _n/a_ | (2-dataset 0.505) |
...
**Rank 4 of 11 entries on the 3-dataset mean.** Beats published BM25 (+0.024), beats GTR-XL (1.2B), Contriever, TAS-B, ColBERT, SBERT. Loses to SPLADE++ (-0.033), GenQ (-0.006 â€” basically tied), and BGE-large (-0.088).

### ArguAna is our weak dataset

ArguAna is counter-argument retrieval â€” the model must understand *opposition* between query and document, not topical similarity. BGE-large dominates here (0.636) because BAAI specifically trained on argument pairs. Our zero-shot BGE-base gets 0.432 â€” same neighborhood as TAS-B (0.429) and GTR-XL (0.439) but well below the top.

The 0.088 mean gap to BGE-large is mostly the ArguAna gap (0.204) â€” on NFCorpus and SciFact we close to 0.022 and 0.039 respectively.

### What did NOT help on ArguAna

- **CE rerank** killed the run (estimated 6+ hours on CPU for 1406 queries Ã— 100-doc rerank). At the 50-query checkpoint nDCG was 0.283 â€” well below dense-alone 0.431 â€” suggesting rerank on counter-argument queries actively hurts. Pulled.
...
| **Xenova/bge-large-en-v1.5 (335M, int8 quantized)** | **0.350** | **no lift** â€” basically tied |
| BAAI/bge-large-en-v1.5 (published, unquantized) | 0.380 | what BAAI reports |

3Ã— the model size, ~3Ã— the embed latency, **no measured quality lift** on this stack. Two likely causes:
...
2. We don't apply BGE's recommended query prefix (`Represent this sentence for searching relevant passages: `) â€” ADR-090 measures that separately
...
```
Tier 0 (NEW): ruvector@0.2.27.embed() â€” bundled, no sharp dep, disk-cache hit
...
(no Tier 4 â€” leave realEmbeddings null, force hash-fallback with explicit _embeddingNote)
```
...
Measured parallel-embedder throughput on this CPU: **6.2Ã— per-doc speedup** (claimed 10-14Ã— in upstream PR #525; ours measured with self-contention from the BEIR benches, so a clean run would land in the 8-12Ã— range).
...
- `src/memory/bge-embedder.ts` â€” adds `embedQuery()` + exports `BGE_QUERY_PREFIX` (ADR-090 opt-in)
- `src/memory/lucene-bm25.ts` â€” Porter stemmer + Lucene stopwords + single-field BM25 (matches published baseline Â±0.003)
- `src/memory/graph-edge-writer.ts` â€” `getBridgeDb({createIfMissing})` (#2246 #4)
- `src/mcp-tools/neural-tools.ts` â€” Tier-0 ruvector probe with content-vs-shape-safe unwrap
- `src/mcp-tools/memory-tools.ts` â€” namespace fan-out fix (#2246 #1) + new `namespaces` param + env override + `namespaceSource` audit field
- `src/mcp-tools/agentdb-tools.ts` â€” pathfinder call sites pass `createIfMissing: true`
- `scripts/run-beir-bge.mjs` â€” `BGE_QUERY_PREFIX=1` env, per-dataset cache path, ArguAna baselines
- `scripts/run-beir-hybrid.mjs` â€” `USE_LUCENE_BM25=1`, `RERANK=1`, `BGE_QUERY_PREFIX=1` flags, ArguAna baselines
- `scripts/run-beir-lucene-bm25.mjs` â€” Lucene BM25 + RRF runner
- `__tests__/memory-search-unified-2246.test.ts` â€” 9 new regression tests
- `package.json` adds `ruvector: ^0.2.27` + root override
- `docs/benchmarks/BEIR-MATRIX.md` â€” 3-dataset rows + per-pipeline comparison
...
- **Our Lucene BM25 matches published baseline Â±0.003.** Re-implementation, not a Lucene binding. Edge cases (numeric tokens, hyphenated terms) may differ.
- **CE rerank does not always help.** Pulled from ArguAna because per-query latency (17s) and per-query nDCG (0.283 at 50q) showed it actively hurts counter-argument retrieval. Pipeline auto-adapts: rerank wins on NFCorpus (0.358 vs 0.360 RRF â€” tied) and SciFact (0.683 vs 0.632 RRF â€” +0.051 lift) but loses on ArguAna.

## What's next (tracked)

- **Tailscale GPU setup** â€” gates the 5 remaining BEIR datasets and any fine-tuning. Blocked on access.
- **Fine-tuning BGE-base on NFCorpus train** (110k pairs, ~3 GPU-hours expected). Blocked on GPU.
- **bge-reranker-v2-m3** (568M, 2.27GB) as an opt-in heavyweight reranker â€” would likely lift NFCorpus + SciFact further.
- **ruvector BGE bundling** ([ruvnet/ruvector#524](https://github.com/ruvnet/ruvector/issues/524)) â€” kills the @xenova/transformers dependency entirely.
- **#2246 finding #2** (postinstall patch-durability checksum) â€” separate release.
...
# 3 BEIR datasets â€” each ingests once, caches, then evals fast on subsequent runs
