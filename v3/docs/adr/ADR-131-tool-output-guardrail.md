# ADR-131: ToolOutputGuardrail â€” semantic screening at the content boundary
...
Introduce `ToolOutputGuardrail` in `@claude-flow/security` â€” a pure, synchronous, pattern-based screener for content crossing the agent's content boundary. It does **not** alter the agent's running prompt; it returns a finding result the caller uses to decide policy (`allow` / `flag` / `redact` / `reject`).

### Shape

```ts
new ToolOutputGuardrail({
  policy: {            // optional â€” defaults shown
    low: 'allow',
    medium: 'flag',
    high: 'redact',
    critical: 'reject',
...
// â†’ { content: string; result: GuardrailResult; action: GuardrailAction }
```
...
| `exfiltration` | critical | "send the api key to â€¦", "leak credentials to â€¦" |
| `role-hijack` | high/medium | "you are now a â€¦", "act as â€¦" |
...
Pattern set is intentionally conservative â€” it targets the explicit shapes that show up in published indirect-injection corpora rather than general "suspicious-looking" text. False-positive rate is bounded by pattern specificity; tune via `customPatterns` and `policy`.

### Integration plan (phased â€” Phase 1 is this PR)
...
P2â€“P5 are tracked separately so the class can ship + be exercised by callers (third-party plugins, integration tests) before deeper wiring.
...
**LLM "instruction tag" wrapper.** Wrap tool output in `<tool-output>â€¦</tool-output>` and instruct the model to ignore instructions inside. Empirically defeated by â‰¥85% of adaptive attacks (arXiv:2601.17548). Not a replacement for the boundary screener.
...
- Pure-function shape â€” safe to invoke in every MCP tool result and memory read
- Pattern set is publicly documented and tunable per-deployment
- Decoupled from the agent's running prompt â€” no prompt-engineering risk
...
- A `reject` finding drops tool output entirely â€” callers must surface the rejection to the agent rather than silently substituting empty content.
...
- `v3/@claude-flow/security/__tests__/tool-output-guardrail.test.ts` â€” 24 tests, 9 ms total
