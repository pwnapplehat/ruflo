...
**Status**: Proposed (tracking only â€” no decisions yet on individual rows)
**Date**: 2026-05-03
...
This ADR is the canonical tracking record for those gaps. Each row below is a candidate for its own follow-up ADR with its own decision, scope, and validation plan. We are not deciding *how* to close any of them here â€” only naming them precisely so they cannot quietly fall off the backlog.

## The Gaps

### G1 â€” `agent_spawn` does not fork a subprocess
...
- Decision: in-process worker pool vs spawned subprocess vs E2B/sandboxes â€” each has a different security/perf profile.
...
### G2 â€” Hive-mind execution is single-process
...
**Why deferred.** Distributed consensus is its own ADR â€” the security and correctness implications cannot be slotted into a /loop iteration.

---

### G3 â€” Workflow execution lacks a runtime
...
### G4 â€” WASM agent prompt echoes input
...
### G5 â€” `@xenova/transformers` â†’ `protobufjs` critical RCE chain
...
**Plan documented.** [ADR-094](./ADR-094-xenova-to-huggingface-transformers-migration.md) â€” try-prefer-fallback loader (`@huggingface/transformers` â†’ `@xenova/transformers`).

**Status.** Implementation landed on branch in iteration #14; verification queued for next /loop publish.

---

### G6 â€” Auto-memory graph state bloat (100 MB / 20 unique entries)
...
- Probably remove PageRank entirely for stores with < 100 unique entries â€” it's not useful at that scale.
...
### G7 â€” Disabled AgentDB controllers (6 of 8 still off after ADR-093 F9)
...
**Why deferred.** Each controller activation is a security decision â€” turning them on in bulk would silently widen the attack surface.
...
- ADR-100: G5 â€” superseded by ADR-094 (this slot left intentionally vacant)
...
This ADR closes when each row above has either landed in its own ADR (proposed/accepted) or been explicitly de-scoped with a recorded reason. The April audit gist ([link](https://gist.github.com/roman-rr/ed603b676af019b8740423d2bb8e4bf6)) is the source of truth for what the audit named â€” re-read it before claiming any G# is done.

## Notes

- ADR-093 was never the right place to address these. It was a punch list of *honesty patches* â€” making the contract match the implementation. ADR-095 explicitly tracks gaps where the implementation needs to expand to match what the contract should be.
...
## Status update â€” 2026-05-11 (post v3.7.0-alpha.21)
...
### G1 â€” agent_spawn â†’ REMEDIATED via `agent_execute` wire

The execution wire shipped as a sibling MCP tool, not by modifying `agent_spawn` (which intentionally remains a registry-only write for cost attribution + swarm coordination). File: `v3/@claude-flow/cli/src/mcp-tools/agent-execute-core.ts:117` â€” `fetch('https://api.anthropic.com/v1/messages', ...)`. Workflow steps go through this wire (see G3).

### G3 â€” workflow_execute â†’ REMEDIATED with real step executor
...
### G4 â€” WASM agent prompt â†’ REMEDIATED with echo-stub detection + Anthropic fallback

`v3/@claude-flow/cli/src/ruvector/agent-wasm.ts:154` (`promptWasmAgent`) detects the bundled WASM agent's `echo: <input>` stub and routes through `callAnthropicMessages` when `ANTHROPIC_API_KEY` is set. When unset, surfaces the stub honestly with a `[NOTE: â€¦set ANTHROPIC_API_KEY to enable real responses]` hint.

### G6 â€” Auto-memory bloat â†’ REFUTED on current main

The 5,706-entries-per-message claim does not match current behavior:
1. Global `~/.claude/settings.json` UserPromptSubmit hook is `[ -n "$PROMPT" ] && npx @claude-flow/cli@latest hooks route --task "$PROMPT" || true` â€” single routing call, no bulk injection.
2. `plugins/ruflo-core/hooks/hooks.json` defines only `PreToolUse`, `PostToolUse`, `PreCompact`, `Stop` â€” no UserPromptSubmit / SessionStart context injection.
3. No `trigram` / `jaccard` symbols in plugin/helper hooks (only one reference in `plugins/ruflo-rag-memory/README.md` documenting MMR diversity reranking â€” different code path).
4. Live measurement: session-start reminder showed `[AutoMemory] âœ“ Imported 0 entries (0 skipped) Backend entries: 8`.
...
### Benchmark integrity â€” REMEDIATED (simulate_benchmarks.py removed)
...
### G2 â€” in progress (consensus transport abstraction)

The first piece landed: `v3/@claude-flow/swarm/src/consensus/transport.ts` introduces a `ConsensusTransport` interface that separates the **inter-node-message** dimension from the **observability-event** dimension. The consensus protocols (raft/byzantine/gossip) historically used a local `EventEmitter` for both â€” the inter-node side never crossed a process boundary (a node "sent" a message by `emit`ting it locally and synthesizing the peer's reply inline).

**Landed (PR #1905, branch `feat/adr-095-g2-hive-mind-ws-transport`):**
- `ConsensusTransport` interface â€” `send` (request-response), `broadcast`, `onMessage`, `peers`, `close`. Separates inter-node messaging from observability events.
- `LocalTransport` â€” in-process registry; the default, matches current single-process behavior. Optional Ed25519 signing + per-sender monotonic-`seq` replay defense.
...
- `FederationTransport` â€” `ConsensusTransport` over the federation plugin's ADR-104 WS wire (`agentic-flow/transport/loader`). Structural dep (swarm stays zero-dep â€” the wiring layer supplies the transport instance). Request-response layered on fire-and-forget WS via correlation ids; ADR-104 stream-mux; Ed25519 + replay defense; fail-closed.
- **All three protocols wired** to accept an injected `transport`: `RaftConsensus` (real RequestVote/AppendEntries RPCs with proper receiver rules â€” term comparison, vote-once-per-term, log-up-to-date check, commitIndex advance), `ByzantineConsensus` (PBFT messages over the transport, sha256 digests replacing the 32-bit toy hash, inbound routing), `GossipConsensus` (gossip messages to neighbors over the transport, inbound merge with dedup-by-id). Legacy no-transport path preserved in all three.
- **BFT fault-tolerance correctness**: `f` now derived from the actual cluster size (`floor((n-1)/3)`, clamped to â‰¥1) rather than hardcoded to 1; `config.maxFaultyNodes` (when set) acts as an upper cap. Quorum `2f+1` checks now use the cluster-derived `f`.
- CI guard: `plugins/ruflo-core/scripts/test-consensus-transport.mjs` (in the `mcp-roundtrip-smoke` job) â€” asserts exports present (incl. `FederationTransport`), `LocalTransport` round-trips, Ed25519 verify is real (no `return true` stub regression).
- Tests: 210/210 swarm suite (+28 new across transport/byzantine/raft/federation/gossip).

Remaining for G2:
- Failure-injection tests (f<n/3 for BFT, f<n/2 for Raft) â€” drive a multi-node `LocalTransport` cluster with simulated faulty/silent nodes; assert correct commits below threshold, no incorrect commits above.
- Cross-host validation (mac â†” ruvultra over tailscale, root) once `FederationTransport` is wired into a real hive-mind + federation setup.
- Build + publish; merge PR #1905.

Tracked in [#1872](https://github.com/pwnapplehat/ruflo/issues/1872) and PR #1905.

### Still open

- **G5** â€” superseded by ADR-094; deps migration status tracked there.
- **G7** â€” controller activation ADRs still per-controller work; 2 of 8 controllers active.
- **#1748** â€” RESOLVED in alpha.22 (ADR-112): 0/285 tools lack "Use when" guidance; CI guard active.

### Tracking

- [#1896](https://github.com/pwnapplehat/ruflo/issues/1896) â€” external audit response with the per-gap measurement evidence above.
