# ADR-088 â€” Lucene-Style BM25 + Cross-Encoder Rerank: The Pipeline That Works

**Status**: Accepted â€” Implemented in ruflo 3.10.28
**Date**: 2026-05-30
...
`src/memory/lucene-bm25.ts` â€” pure-function module, no external deps:

- **Porter stemmer (1980)** implemented inline, ~150 LOC, 12/12 published test cases passing (`caresses â†’ caress`, `agreed â†’ agre`, `motoring â†’ motor`, `vietnamization â†’ vietnam`, etc.).
- **Lucene 8.x English stopword list** (~120 tokens â€” Lucene's default 33 plus BEIR-conventional extras).
...
- `USE_LUCENE_BM25=1` â€” swap multi-field BM25 for Lucene-style
- `RERANK=1` â€” apply `Xenova/ms-marco-MiniLM-L-6-v2` cross-encoder over top-100 RRF output
...
Ruflo's runtime retrieval still uses the multi-field BM25 + dense + MMR + optional CE rerank pipeline from ADRs 078-083, tuned against repo-history corpora. The Lucene BM25 in this ADR is a **BEIR-benchmark-only** module â€” the multi-field BM25 stays better for short commit-subject text. We isolated the benchmark-vs-runtime concerns deliberately.

## Measured proof â€” full ablation matrix (N=323 NFCorpus, N=300 SciFact)

| Configuration | NFCorpus nDCG@10 | SciFact nDCG@10 | Mean | Beats published BM25 both? |
|---|---:|---:|---:|---|
| dense alone (BGE-base) | 0.352 | 0.626 | 0.489 | âœ— (loses SciFact -0.053) |
| Multi-field BM25 alone | 0.279 | 0.576 | 0.428 | âœ— (loses both) |
| **Lucene BM25 alone (ADR-088)** | **0.328** | **0.681** | **0.505** | **tied** (NFCorpus +0.003, SciFact +0.002) |
| Multi-field RRF k=60 (ADR-087, broken) | 0.328 â†“ | 0.569 â†“ | 0.449 | âœ— (loses both) |
| **Lucene RRF k=60** | 0.360 | 0.632 | 0.496 | âœ— (loses SciFact -0.047) |
| **Lucene RRF k=30** | **0.363** | 0.639 | 0.501 | âœ— (loses SciFact -0.040) |
| Multi-field RRF k=60 + CE rerank | 0.355 | **0.685** | **0.520** | âœ“ (NFCorpus +0.030, SciFact +0.006) |
| **Lucene RRF k=60 + CE rerank (best)** | **0.358** | **0.683** | **0.521** | âœ“ (NFCorpus +0.033, SciFact +0.004) |
| BM25 (published Lucene) | 0.325 | 0.679 | 0.502 | â€” |
| SPLADE++ (published) | 0.347 | 0.704 | 0.526 | â€” |
| BGE-large-v1.5 (published) | 0.380 | 0.722 | 0.551 | â€” |
...
**Acceptance test from the climb plan ("ruflo beats BM25 on both datasets") PASSES.** With RRF+CE rerank we're 0.521 on the 2-dataset mean â€” beats published BM25 (0.502), beats every other published baseline except SPLADE++ (0.526, 1 percentage point above us) and BGE-large (0.551, 3 percentage points above).

### Notable per-dataset ranks (Lucene RRF + CE rerank)

- **NFCorpus: 0.358, rank 2 of 11** â€” only behind BGE-large (0.380). Beats SPLADE++ (0.347) by 0.011.
- **SciFact: 0.683, rank 3 of 11** â€” only behind SPLADE++ (0.704) and BGE-large (0.722). Beats every other listed dense baseline including Contriever, DocT5query, ColBERT, GTR-XL, GenQ, TAS-B.

### Subtle finding from the ablation

On NFCorpus, **Lucene RRF k=60 alone (0.360) is essentially tied with Lucene RRF + CE rerank (0.358)** â€” the cross-encoder doesn't help when the underlying RRF is already strong. The CE rerank's value is on SciFact (0.639 â†’ 0.683, +0.044 lift). The pipeline auto-adapts: when RRF is strong, rerank is mostly a pass-through; when RRF is weaker, rerank substantially lifts.

This matches the published literature on hybrid retrieval â€” reranking helps most when the candidate pool has high recall but low top-K precision.

## What this validates

1. **The ADR-087 diagnosis was correct.** RRF works with comparably-strong inputs and breaks with asymmetric strength. Lucene BM25 + dense â†’ RRF works (lifts both datasets). Multi-field BM25 + dense â†’ RRF degrades (asymmetric strength).
...
- **Latency cost of rerank is real** â€” ~4.6 sec per query on this M-series CPU at top-100 rerank. Production callers should opt in based on their latency budget.
...
- **Our Lucene BM25 is a re-implementation, not a Lucene binding.** Matches the published baseline within Â±0.003 on both datasets â€” close, not identical. Lucene's actual implementation may differ in edge cases (numeric handling, hyphenated tokens, etc.).
...
- **BGE-large swap** â€” drop-in `BGE_MODEL=Xenova/bge-large-en-v1.5`. Likely lifts both datasets further. ~3Ã— embed latency.
- **TREC-COVID + FiQA + ArguAna** â€” 3-4 more BEIR datasets with the Tailscale GPU path the user offered. Would establish a real BEIR-mini-average.
- **Fine-tuning BGE-base on NFCorpus train** (110K-pair train split) â€” GPU job, +0.02-0.05 expected.
- **ruvector BGE bundling** (ruvnet/ruvector#524) â€” kills the silent-fallback bug at source.
...
# â†’ nDCG@10 0.358, rank 2/11 on NFCorpus
...
# â†’ nDCG@10 0.683, rank 3/11 on SciFact
